...
parser.add_argument(
    '-p', '--poll', help='How long the script should sleep before checking status')
...
r = requests.post('https://circleci.com/api/v1/project/{}/tree/{}?circle-token={}'.format(
    repo, branch, token), json=build_parameters)
...
status = requests.get(
    'https://circleci.com/api/v1.1/project/github/{}/{}?circle-token={}'.format(repo, r.json()['build_num'], token))
